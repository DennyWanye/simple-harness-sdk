...
| S1 Semantic relation mutation wire | Harness candidate 已完成 | mutation schema v5 冻结 claim/relation discriminator 与唯一 `applies_to`；relation endpoint 支持 existing/created exact ref，created ref 必须显式 dependency 指向 CREATE，且只允许 Semantic claim → Procedure/Prospective。Memory 持久化、graph projection 与 Host durable diagnostic audit 仍是跨仓门禁。 |
