...
- 测试：`tests/orchestrator/full_target` **2690 条**（上一段 2648）；旧模式回归 1 failed / 1966 passed / 20 skipped（唯一的红为已知的 Python 3.14 `ast.dump` 漂移）。真实模型冒烟跑到了但**未到 COMPLETED**：Planner 轮可读、plan revision 1 提交、`accepted_outputs` 非空（D3 的活证据，收口断言通过），8 次 attempt 被端点超时/5xx 吃光后 `budget_exhausted`；端点整轮抽风且不提供 `deepseek-flash`，详见 journal 第四部分 §2c。
