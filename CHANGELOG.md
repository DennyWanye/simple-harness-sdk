...
- **有界 / 诚实停止**：仍受 `max_root_review_repairs`（每 revision，默认 1）与既有合成上限约束；`fail_mission(NO_DISPATCHABLE_WORK)` 的 detail 多 `root_review{reason: root_review_rejected, status, package_id, plan_revision, repairs_used, max_root_review_repairs, findings}`。
...
- 口径：Planner / 合成器看到的 JSON 变了（新段、新字段），`context_version` 因而不同；runner FREEZE-candidate 需重生成；契约变更请求：无。未做：真实模型上跑通「REJECT → 换方法或合成 → ACCEPT」；被拒历史不跨修订累计（替换方法再被拒时第一个被拒方法会重回候选，默认 `max_root_review_repairs=1` 下走不到）。
...
- 口径：根评审员看到的 JSON 多两个字段、合成器 `operators` 少 @1 行、默认提示词 root-reviewer-v3 / method-synthesizer-v5（旧版可 pin）、分层叶 `verification_policy` 变（只读叶无 code_test）；runner FREEZE-candidate 需重生成。**无新增配置项**，策略快照 digest 不变，legacy 事件字节不变。未做：真实模型上四项均未验（第 3 批）；Host 侧 H1 / D2a 未动。详见 journal 第四部分 §2m。
