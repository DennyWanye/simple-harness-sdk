...
- **提示词**：`planner-hierarchical-v5`（自 v4，说明 `rejected_refinements` 与 retire_method+refine 修复写法；包版本 3 只登记 v5，钉 v1–v4 的部署回落 v5）、`method-synthesizer-v3`（自 v2，说明 `review_feedback`：必须不同于被拒方法、要回答 findings，例如先写一条会失败的测试）；旧版保留、sha256 钉住（`FROZEN_PROMPT_DIGESTS` +2）。
