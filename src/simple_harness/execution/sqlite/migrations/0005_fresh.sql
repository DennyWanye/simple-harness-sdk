...
CREATE TABLE conversation_command_namespaces (
    namespace TEXT PRIMARY KEY,
    projection_key_id TEXT NOT NULL,
    created_at REAL NOT NULL CHECK(created_at >= 0)
) STRICT;

CREATE TABLE conversation_run_modes (
    run_id TEXT PRIMARY KEY,
    namespace TEXT NOT NULL REFERENCES conversation_command_namespaces(namespace),
    api_mode TEXT NOT NULL CHECK(api_mode IN ('legacy','command')),
    intent_hash TEXT NOT NULL CHECK(length(intent_hash) = 64),
    created_at REAL NOT NULL CHECK(created_at >= 0)
) STRICT;

CREATE TABLE conversation_command_streams (
    run_id TEXT PRIMARY KEY REFERENCES conversation_run_modes(run_id),
    namespace TEXT NOT NULL REFERENCES conversation_command_namespaces(namespace),
    next_accept_seq INTEGER NOT NULL DEFAULT 1 CHECK(next_accept_seq >= 1),
    cancel_fence_seq INTEGER CHECK(cancel_fence_seq IS NULL OR cancel_fence_seq >= 1)
) STRICT;
...
CREATE TABLE conversation_commands (
    ...
    namespace TEXT NOT NULL REFERENCES conversation_command_namespaces(namespace),
    projection_key_id TEXT NOT NULL,
    run_id TEXT NOT NULL REFERENCES conversation_command_streams(run_id),
    kind TEXT NOT NULL CHECK(kind IN ('start','continue','cancel')),
    accept_seq INTEGER NOT NULL CHECK(accept_seq >= 0),
    intent_hash TEXT NOT NULL CHECK(length(intent_hash) = 64),
    raw_payload_json TEXT,
    state TEXT NOT NULL CHECK(state IN (
        'accepted','context_call_intent','context_ready','applied','rejected','cancelled'
    )),
    version INTEGER NOT NULL DEFAULT 1 CHECK(version >= 1),
    owner_id TEXT,
    claim_epoch INTEGER NOT NULL DEFAULT 0 CHECK(claim_epoch >= 0),
    lease_expires_at REAL,
    attempt_count INTEGER NOT NULL DEFAULT 0 CHECK(attempt_count >= 0),
    retry_at REAL NOT NULL DEFAULT 0 CHECK(retry_at >= 0),
    last_error_code TEXT,
    created_at REAL NOT NULL CHECK(created_at >= 0),
    updated_at REAL NOT NULL CHECK(updated_at >= 0),
    UNIQUE(run_id, accept_seq),
    CHECK((state IN ('applied','rejected','cancelled')) = (raw_payload_json IS NULL)),
    CHECK((owner_id IS NULL) = (lease_expires_at IS NULL))
) STRICT;

CREATE INDEX conversation_commands_ready
ON conversation_commands(state, retry_at, lease_expires_at, run_id, accept_seq);

CREATE TABLE conversation_outputs (
    run_id TEXT PRIMARY KEY REFERENCES runs(run_id),
    command_id TEXT NOT NULL UNIQUE REFERENCES conversation_commands(command_id),
    output_json TEXT NOT NULL,
    output_hash TEXT NOT NULL CHECK(length(output_hash) = 64),
    created_at REAL NOT NULL CHECK(created_at >= 0)
) STRICT;

CREATE TRIGGER conversation_namespace_binding_immutable
BEFORE UPDATE OF projection_key_id ON conversation_command_namespaces
BEGIN SELECT RAISE(ABORT, 'conversation namespace binding is immutable'); END;

CREATE TRIGGER conversation_command_identity_immutable
BEFORE UPDATE OF namespace,projection_key_id,run_id,kind,accept_seq,intent_hash ON conversation_commands
BEGIN SELECT RAISE(ABORT, 'conversation command identity is immutable'); END;
