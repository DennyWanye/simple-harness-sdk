...
class WorkflowCheckpointPort(Protocol):
    ...
    async def commit(
        self, checkpoint: WorkflowCheckpoint, *, expected_revision: int | None,
        lease: WorkflowLease, transaction: WorkflowTransaction,
    ) -> WorkflowCheckpoint: ...

    async def fork(
        self, *, source: WorkflowCheckpoint, target_run_id: str,
        values: Mapping[str, JsonValue], transaction: WorkflowTransaction,
    ) -> WorkflowCheckpoint: ...
...
class SqliteNativeCheckpointStore:
    ...
    def _assert_writer(
        ...
        *,
        ...
        latest = connection.execute(
            "SELECT checkpoint_id FROM workflow_checkpoints WHERE run_id=? AND namespace=? ORDER BY version DESC LIMIT 1",
            (run_id, namespace),
        ).fetchone()
    ...
    def _write_native_operation(
        ...
        *,
        ...
        payload_json = json.dumps(detached, ensure_ascii=False, allow_nan=False, separators=(",", ":"), sort_keys=True)
        ...
        connection.execute(
            ...
            """,
            (operation_id,run_id,namespace,base_checkpoint_id,operation_kind,
             identity_key,payload_json,payload_hash,now),
        )
        return detached
    ...
    def _insert_snapshot(
        ...
        *,
        ...
        checkpoint_json = json.dumps(raw, ensure_ascii=False, allow_nan=False, separators=(",", ":"), sort_keys=True)
        connection.execute(
            """
            ...
            """,
            (snapshot.checkpoint_id,snapshot.run_id,snapshot.checkpoint_ns,checkpoint_json,
             hashlib.sha256(checkpoint_json.encode()).hexdigest(),lease_epoch,version,now),
        )
    ...
    async def ensure_genesis(self, *, operation_id: str, snapshot, configurable):  # type: ignore[no-untyped-def]
        async def operation(transaction: WorkflowTransaction):  # type: ignore[no-untyped-def]
            ...
            _, epoch, now = self._assert_writer(
                connection,run_id=snapshot.run_id,namespace=snapshot.checkpoint_ns,
                expected_head=None,configurable=configurable,
            )
            self._write_native_operation(
                connection,operation_id=operation_id,run_id=snapshot.run_id,
                namespace=snapshot.checkpoint_ns,base_checkpoint_id=snapshot.checkpoint_id,
                operation_kind="genesis",identity_key=snapshot.checkpoint_id,
                payload={"snapshot_hash": hashlib.sha256(json.dumps(snapshot.to_dict(), sort_keys=True).encode()).hexdigest()},now=now,
            )
            self._insert_snapshot(connection,snapshot=snapshot,lease_epoch=epoch,version=0,now=now)
            await self._execution_ports.checkpoint.mark_running_on_claim(
                transaction,run_id=snapshot.run_id,checkpoint_namespace=snapshot.checkpoint_ns,
                lease_epoch=epoch,claim_epoch=epoch,now=now,
            )
            return snapshot
    ...
    async def load_execution(self, *, run_id: str, thread_id: str, checkpoint_ns: str):  # type: ignore[no-untyped-def]
        ...
        operations = self._database.connection.execute(
            "SELECT operation_kind,identity_key,payload_json FROM workflow_native_operations WHERE run_id=? AND namespace=? AND base_checkpoint_id=? ORDER BY created_at,operation_id",
        ...
        for operation in operations:
            ...
            if kind == "task_result":
                ...
                retries[key] = (cast(int, payload["retry_attempt"]), cast(float, payload["next_attempt_at"]))
            elif kind == "interrupt":
        ...
        if retries:
            snapshot = replace(snapshot, frontier=tuple(
                replace(task,retry_attempt=retries[task.task_id][0],next_attempt_at=retries[task.task_id][1])
                if task.task_id in retries else task for task in snapshot.frontier
            ))
        ...
        async def operation(transaction: WorkflowTransaction):  # type: ignore[no-untyped-def]
            ...
            if existing is not None:
                return self._write_native_operation(
                    ...
                    operation_kind=kind,identity_key=identity_key,payload=payload,
    ...
    async def commit_task_result(self, *, operation_id: str, expected_head: str, task, execution_info, patch, blob_refs, consumed_interrupt_ids, configurable):  # type: ignore[no-untyped-def]
        await self._commit_operation(
            operation_id=operation_id,expected_head=expected_head,kind="task_result",
            identity_key=task.task_id,payload={"patch": patch.to_dict(),"first_attempt_time": execution_info.node_first_attempt_time,"consumed_interrupt_ids": list(consumed_interrupt_ids)},
            configurable=configurable,blob_refs=blob_refs,
        )

    async def commit_route_selection(self, *, operation_id: str, expected_head: str, source: str, selected_route: str, next_frontier_payload_hash: str, task_id: str, configurable):  # type: ignore[no-untyped-def]
        selection = cast(
            dict[str, JsonValue],
            {
                "source": source,
        ...
        saved = await self._commit_operation(
            operation_id=operation_id,expected_head=expected_head,kind="route",
            identity_key=task_id,payload={"selection":selection},configurable=configurable,
        )
    ...
    async def commit_interrupt(self, *, operation_id: str, expected_head: str, task, interrupt, configurable):  # type: ignore[no-untyped-def]
        ...
        async def commit_and_open(transaction: WorkflowTransaction):  # type: ignore[no-untyped-def]
            ...
            saved = self._write_native_operation(
                connection,operation_id=operation_id,run_id=run_id,namespace=namespace,
                base_checkpoint_id=expected_head,operation_kind="interrupt",
                identity_key=task.task_id,payload={"interrupt":request},
    ...
    async def read_terminal_projection_prepare(self, *, operation_id: str, expected_head: str, configurable):  # type: ignore[no-untyped-def]
        ...
        async def operation(transaction: WorkflowTransaction):  # type: ignore[no-untyped-def]
            connection = self._connection(transaction)
            row = connection.execute("SELECT * FROM terminal_projection_prepares WHERE operation_id=?",(operation_id,)).fetchone()
    ...
    def _projection_receipt(row):  # type: ignore[no-untyped-def]
        ...
        return TerminalProjectionPrepareReceipt(
            operation_id=str(row["operation_id"]),run_id=str(row["run_id"]),
            terminal_checkpoint_id=str(row["terminal_checkpoint_id"]),
            descriptor_digest=str(row["descriptor_digest"]),input_hash=str(row["input_hash"]),
            output=cast(dict,json.loads(str(row["output_json"]))),output_hash=str(row["output_hash"]),
            blob_refs=tuple(cast(list[str],json.loads(str(row["blob_refs_json"])))),
        )

    async def prepare_terminal_projection(self, *, operation_id: str, expected_head: str, descriptor_digest: str, input_hash: str, output, output_hash: str, blob_refs, configurable):  # type: ignore[no-untyped-def]
        run_id = cast(str, configurable["run_id"])
        namespace = cast(str, configurable.get("checkpoint_ns", ""))
        async def operation(transaction: WorkflowTransaction):  # type: ignore[no-untyped-def]
            connection = self._connection(transaction)
            existing = connection.execute("SELECT * FROM terminal_projection_prepares WHERE operation_id=?",(operation_id,)).fetchone()
            output_json = json.dumps(dict(output),ensure_ascii=False,allow_nan=False,separators=(",",":"),sort_keys=True)
            ...
            if existing is not None:
                ...
                if (receipt.run_id,receipt.terminal_checkpoint_id,receipt.descriptor_digest,receipt.input_hash,receipt.output_hash,str(existing["output_json"]),receipt.blob_refs)!=(run_id,expected_head,descriptor_digest,input_hash,output_hash,output_json,refs):
                ...
                return receipt
            owner,epoch,now = self._assert_writer(connection,run_id=run_id,namespace=namespace,expected_head=expected_head,configurable=configurable)
            await self._blob_references.validate_references(transaction,run_id=run_id,owner_kind="terminal_projection",owner_id=operation_id,blob_refs=refs)
            connection.execute(
                ...
                (operation_id,run_id,namespace,expected_head,descriptor_digest,input_hash,
                 output_json,output_hash,json.dumps(list(refs),separators=(",",":")),owner,
                 epoch,expected_head,now),
            )
            return self._projection_receipt(connection.execute("SELECT * FROM terminal_projection_prepares WHERE operation_id=?",(operation_id,)).fetchone())
        return await self._atomic("projection_prepare", operation)

    async def commit_frontier(self, *, operation_id: str, expected_head: str, state, frontier, completed_activations, join_firings, consumed_interrupt_ids, intents, blob_refs, terminal_status, terminal_error, recovery_action, terminal_projection_prepare_id, configurable):  # type: ignore[no-untyped-def]
        ...
        request_json = json.dumps(
            request_payload,ensure_ascii=False,allow_nan=False,separators=(",",":"),sort_keys=True
        )
        request_hash = hashlib.sha256(request_json.encode()).hexdigest()
        async def operation(transaction: WorkflowTransaction):  # type: ignore[no-untyped-def]
            connection = self._connection(transaction)
            existing_operation = connection.execute("SELECT payload_json FROM workflow_native_operations WHERE operation_id=?",(operation_id,)).fetchone()
            if existing_operation is not None:
                replay = _strict_object(json.loads(str(existing_operation["payload_json"])), "frontier_receipt")
                ...
                row = connection.execute("SELECT * FROM workflow_checkpoints WHERE checkpoint_id=?",(next_id,)).fetchone()
                if row is None: raise WorkflowOperationConflict("frontier receipt lacks checkpoint")
                snapshot = self._snapshot_from_row(row)
                return NativeCommitResult(snapshot,tuple(cast(list[str],replay["event_ids"])))
            _,epoch,now = self._assert_writer(connection,run_id=run_id,namespace=namespace,expected_head=expected_head,configurable=configurable)
            current_row = connection.execute("SELECT * FROM workflow_checkpoints WHERE checkpoint_id=? AND run_id=? AND namespace=?",(expected_head,run_id,namespace)).fetchone()
            ...
            if (
                current_logical_time != configurable.get("logical_timestamp")
            ...
            if terminal_projection_prepare_id is not None:
                ...
                projection_request = NativeWorkflowExecutable._terminal_projection_request(
                    descriptor=descriptor,state=state,run_id=run_id,
                    workflow_name=cast(str, configurable.get("workflow_name")),
                    workflow_version=cast(str, configurable.get("workflow_version")),
                    status=cast(str, terminal_status),error=terminal_error,
                    recovery_action=recovery_action,
                )
                projection_input_hash = hashlib.sha256(
                    json.dumps(projection_request,ensure_ascii=False,allow_nan=False,separators=(",",":"),sort_keys=True).encode()
                ).hexdigest()
                ...
                expected_terminal_intents = NativeWorkflowExecutable.terminal_intents(
                    projected_state,run_id=run_id,status=terminal_status,
                    error=terminal_error,recovery_action=recovery_action,
                )
                expected_mappings = [
                    {
                ...
                if json.dumps(expected_mappings,sort_keys=True,separators=(",",":")) != json.dumps(
                    [dict(intent) for intent in intents],sort_keys=True,separators=(",",":"),
                ):
            ...
            await self._blob_references.validate_references(transaction,run_id=run_id,owner_kind="workflow_checkpoint",owner_id=operation_id,blob_refs=blob_refs)
            next_id = _stable_id(run_id,operation_id)
            metadata = copy.deepcopy(dict(current.metadata))
            snapshot = NativeSnapshotEnvelope(
                thread_id=current.thread_id,checkpoint_ns=namespace,checkpoint_id=next_id,
                parent_checkpoint_id=current.checkpoint_id,run_id=run_id,
                state_schema_version=current.state_schema_version,step=current.step+1,
                state=copy.deepcopy(dict(state)),frontier=tuple(frontier),
                completed_activations=dict(completed_activations),join_firings=tuple(join_firings),
                metadata=metadata,
            )
            next_version=int(current_row["version"])+1
            self._insert_snapshot(connection,snapshot=snapshot,lease_epoch=epoch,version=next_version,now=now)
            event_ids=[]
            for intent in intents:
                intent_id=cast(str,intent.get("event_key",intent.get("intent_id")))
                outcome=await self._execution_ports.checkpoint.materialize_intent(transaction,run_id=run_id,intent_id=intent_id,intent=intent,checkpoint_namespace=namespace,checkpoint_id=next_id,lease_epoch=epoch,now=now)
                assert isinstance(outcome,dict); event_ids.append(cast(str,outcome["event_id"]))
            if consumed_interrupt_ids:
                responses={}
                for decision_id in consumed_interrupt_ids:
                    row=connection.execute("SELECT response_json FROM decisions WHERE decision_id=?",(decision_id,)).fetchone()
                    if row is None or row["response_json"] is None: raise WorkflowOperationConflict("consumed decision has no response")
                    responses[decision_id]=json.loads(str(row["response_json"]))
                await self._execution_ports.checkpoint.consume_decisions(transaction,run_id=run_id,checkpoint_id=next_id,decision_ids=consumed_interrupt_ids,responses=responses,checkpoint_namespace=namespace,lease_epoch=epoch,now=now)
            if terminal_projection_prepare_id is not None:
                if prepare_row is None:
                    raise WorkflowOperationConflict(
                        "terminal projection prepare disappeared before consume"
                    )
                await self._blob_references.validate_references(transaction,run_id=run_id,owner_kind="terminal_projection",owner_id=terminal_projection_prepare_id,blob_refs=tuple(json.loads(str(prepare_row["blob_refs_json"]))))
                connection.execute("UPDATE terminal_projection_prepares SET consumed_at=? WHERE operation_id=? AND consumed_at IS NULL",(now,terminal_projection_prepare_id))
