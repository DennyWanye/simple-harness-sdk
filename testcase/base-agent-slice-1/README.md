...
| AC2 结果回到主 Agent | 决定性 | 同上 + `tests/agents/test_delegation_e2e_mock.py::test_delegation_end_to_end_with_restart` + 真实模型 `reports/` | NONCE 只在子模板里；主最终 `public_output` 含 NONCE；工具结果携带子 result_hash |
...
| AC9 委派 UNKNOWN 结算 | 可选 | `test_delegate_tool.py::test_delegate_unknown_reconciles_from_child_result_row` | `observe` 凭子结果行返回 COMPLETED（evidence_ref = 子 commit_receipt_id）或 STILL_UNKNOWN（pending ref），从不 launch |
| AC10 真实模型复现 | 可选 | `tests/agents/test_delegation_e2e_real_provider.py`（`--run-real-provider`） | 见 journal §4.3 报告 |
| 子 Agent 不能递归、未暴露工具被拒 | — | `test_delegate_tool.py::test_child_cannot_delegate_and_unexposed_tool_is_rejected` | 执行期闸门（review F1） |
...
| 创建不驱动模型、意外 continuation 不杀 Agent、预算超限=失败 Turn、provider 拒绝可恢复、未知工具名可见拒绝、轮内 UNKNOWN 阻塞后 reconcile 续跑、finalize 异常不终态、首轮重跑不重复消息 | `tests/agents/test_agent_driver.py` |
