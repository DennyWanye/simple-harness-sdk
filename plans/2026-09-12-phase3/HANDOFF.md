## 2026-09-14 final source checkpoint — supersedes historical open lists

Current original48AC matrix: **46 SOURCE PASS /0 OPEN /2 user-deferred packaging criteria**. All46 approved source-scope criteria have evidence closure; no installer/release or larger complete-design completion claim. See p36/current-source-audit.md. Production SDK ae8d37b / Host18ff5d24; later commits record conclusions. No packaging, installer, release or push.

Last remaining gate P3.4-A04 passed one fixed v15 FIRST/COMPARE pair on immutable snapshot-v41 with actual deepseek-flash. Both strictPASS/COMPLETED: FIRST76calls645304tokens423.613seconds; COMPARE112calls1273253tokens605.314seconds. Total188calls1918557tokens, runner1029.73seconds; zero physical Provider errors/unknown usage/reserve/rehandoff, source336inputs unchanged, processes exited. See p34/v15-real-pair-review.md for candidate/synthesis/repair/fragment IDs and hashes. Do not rerun a paid pair merely to seek a more favorable cost result. One pair does not prove model-quality superiority; COMPARE used more tokens here.

The justified correction after v14 is explicit tool_schema_mode=deepseek-strict-v1 at official Beta, equivalent bounded strict schemas, matching counter/HTTP serializer and distinct frozen pool deepseek-strict-context-256k-v1. P34 profile context256-8m-strict32k-v9 uses initial/ceiling32768 output,262144context, same8M/24 and all Task goals/materials/criteria/budgets. Output32K is not context32K. Legacy wire/fingerprint and old pools remain unchanged; strict is not automatically wired into existing Host defaults. See p34/strict-provider-mode.md for supported schema boundaries. v15 is real API/source acceptance, native_ui=NOT_RUN; do not relabel old native evidence as this new strict mode.

Committed SDK affected checks158PASS6.76seconds, ruff/mypyPASS. Prior full-source baseline SDK2129PASS with22initial failures closed by affected86PASS; Host307PASS with3wheel-only and1default-real deselected. This is not a new all-suite invocation. Native-v36 publication/human retry/cold, native-v37 genuine old PLANNING recovery, native-v39 code/actual Critic test output/pending human cold approval retain their source-specific evidence. Actual256K/512K context and multi-turn/cold acceptance is separately documented. Historical v13/v14 strictFAIL records remain unchanged.

Next work in the approved source acceptance list: none. P3.1-A08 installed-version identity and P3.6-A07 artifact-level cumulative acceptance stay DEFERRED because the user explicitly postponed packaging. Do not build/install/release/push without a new user scope instruction. User may next exercise current source UI; existing source launcher owns Tauri/backend/Vite, and credentials remain read_key()/Keychain only.

Orchestration: NO plan-test/plan-task/plan-bs skills. Main Astra/high fixed, up to3 adaptive scoped children allowed. Parent sole test/API/native runner; no active child/pair now. Actual final-review resumed child used Astra/high after an initial Sol/medium turn; this is recorded with cache-separated counters, not mislabelled Sol. Parent overhead not separately timed and no savings percentage claimed. Raw evidence remains ignored; no NAS deletion. Caffeinate retained per user request.

## Historical checkpoints — not the current remaining-work list

## 2026-09-14 current checkpoint

P36 Host diagnostics PD1–PD4 native/cold accepted Hostd093f55c, recorded Host90ec0b5c; no packaging. SDK P34 v11 originalstrictFAIL, both Missions delivered, details p34/v11-real-pair-review.md. Bounded prompt/observer fix44PASS8.86s and independentreviewACCEPT; committed/newreal gates pending. Real512Kmulti-turn2calls1045085tokens42.987s and cold0calls PASS againstSDKdc2f156. LC2 truepre-context coexistence and final cumulative remainOPEN. Continue original Phase3, main Astra/high, <=3adaptivechildren; parent sole runner.

**当前范围与交接覆盖历史记录：2026-09-14。** 用户已要求继续整个Phase3功能，P3.6功能属于本轮；只暂缓打包/安装制品/发布/推送。P33历史批准功能通过但受影响最终累计需更新；P34真实pair v10仍失败，P35-A04重开，长Context256/512的新软件/原生证据已取得但不能按旧8/8宣称当前全完成。P36回放/归因/脱敏部分已验证，其余门继续。最新见p34/long-context-results.md、p36/functional-acceptance.md；新对照合同context256-pair-contract.md，不重写旧2M失败。
