## 2026-09-14 source acceptance checkpoint - supersedes older open lists

Current matrix: **45 SOURCE PASS /1 strict P34 OPEN /2 packaging criteria DEFERRED**, original48AC; see p36/current-source-audit.md. Production SDK7f926b2 / Host18ff5d24, native snapshot-v39. Later SDK8a48e3e/3eebcd1 and Hostad5ab000 are test/evidence updates. No packaging, installer, release or push.

Native-v36 local publication/human retry notes and cold reads passed. Native-v37 normal old PLANNING frozen input/config and first Planner settle passed, old default preserved; actual old/new report cold reads passed. v37 code Mission failed because Critic received no test output; source now runs required verification-copy pytest before Critic and preserves real test evidence. Host model-slot waiting no longer appears as UNKNOWN. Native-v39 two code Missions passed actual seatbelt pytest, one explicitly required Critic and its actual requests contained test output. Both pending reviews survived cold with identical selected tables, then UI-approved to delivery,10 real calls26932tokens/no new model calls after cold approval. Detailed p32/native-v39-review.md.

Source cumulative: SDK2129PASS with22initial failures closed by affected86PASS (tiktoken environment21 + obsolete attribution1); production identical. Initial context_journal module skip tested;12 real opt-ins separate. Host307PASS/4deselected (3wheel-only +1default real marker). See p36/source-cumulative-v39.md and Host source-cumulative-v39.md. Original failures retained; not one all-green invocation claim.

The bounded fixed-source real pair v13 has FINISHED (no active test process). COMPARE strictPASS101calls998962tokens556.739s; FIRST COMPLETED but strictFAIL56calls544122tokens322.582s due one tool_parse response with finish_reason=tool_calls/output1347. Total157calls1543084tokens, runner880.20s; both reserve0/rehandoff0. Read p34/v13-real-pair-review.md before any new paid run. Do not reroll until success or discard the FIRST failure. Safe finite parse-reason diagnostics subsequently passed67tests1.90s, with AgentTurn persistence/cold-read; see p34/tool-parse-diagnostics-v40.md. They are a small SDK successor to snapshot-v39 and have no new native UI claim. Precise historical v13 parse cause is unavailable. Next work requires targeted new evidence/correction, then a justified fixed pair; direct Beta strict switching is incompatible with current optional tool fields and has not been implemented.

User orchestration: NO plan-test/plan-task/plan-bs skills. Main Astra/high remains fixed; up to3 adaptive scoped children allowed, not mandatory. Parent is sole pytest/native/Provider runner. Latest children all closed, outcomes/cached-vs-uncached counters in Host agent-model-observations.md. Credentials only read_key()/Keychain, never print or persist; app Provider deepseek-flash. Raw evidence stays ignored, no NAS deletion. Caffeinate retained for native tests.
