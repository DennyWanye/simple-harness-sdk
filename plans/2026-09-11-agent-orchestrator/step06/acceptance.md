...
附加门槛：`tests/orchestrator`（step02–06）全绿；SDK 全量红集 ⊆ 基线；安装 wheel 后跑 `tests/orchestrator` 与 `python -m agent_orchestrator demo --scenario multi-mission --provider fixtures --evidence-dir evidence/s6`；真实多执行池小规模负载报告（用户 2026-09-11 指示"模型都使用 deepseek-flash，不要再用 deepseek-v4-pro"：两个执行池 `small` / `large` 都是 deepseek-flash，只在输出上限与执行池上不同；物理路由由各池独立执行库与回显证明，"升级"是换到更宽输出上限的池，不换模型名）；独立 review 处置；推送 origin main 且本地干净。
