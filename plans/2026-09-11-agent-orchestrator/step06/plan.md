...
"做"= 本步实现并有测试；"前序"= 第 2–5 步已有，本步沿用或只加闸门；"登记"= 按 ORCH §13 登记的实施约定或降级；"不做"= 归属后续步骤。

| 规则 | 原文 | 本步处置 |
|---|---|---|
| 3.1 | §8.1 Frontier | 前序（`frontier()`）；RAISED 时在 Frontier 之上加闸门（D6-3） |
| 3.2 | §8.2 Allocator 决定用什么模型 | 做：Model Router 在派发时决定 profile 并冻结（D6-4，S6-03） |
| 3.3 | §8.2 下游积压惩罚 | 登记：以公式外的闸门实现，不给 §29.3 加系数（§6.1） |
| 3.4 | §8.2 角色 × 数量分配 | 不做：配比只观测（D6-11）；按角色数量分配待第 8 步评测 |
| 3.5 | §8.3 在哪个 Worker 上运行 | 做：执行池（D6-5，S6-08）；超时/重试/取消冗余为前序 |
| 3.6 | §8.3 Allocator 与 Scheduler 分工 | 做：背压改 Allocator 的"给多少"，全局运行上限在 `create_attempt` 事务里管"此刻多快" |
| 3.7 | §9.1 搜索多样性 | 不做：多样性配额登记（§6.1），第 8 步评测 |
| 3.8 / 3.9 | §9.2 八角色、角色是搜索偏置 | 前序（第 5 步模板）；本步 `metrics.json` 按八角色报告配比 |
| 3.10 | §9.3 按任务类型路由 | 做：`RoutingRules.by_task_kind`（S6-06 测试用 `docs` 类） |
| 3.11 | §9.3 升级阶梯 | 做：`escalate`（S6-03；真实运行 1 实际发生 small → large） |
| 3.12 | §10 上下文第 9/10 项 | 部分：模型只被提供交集内的工具（S6-05 断言 provider 请求）；预算信息沿用第 2 步包 |
| 3.13 | §10.1 检索 | 前序（第 4 步）；本步断言检索只查本 Mission（S6-01） |
| 3.14 | §10.2 可见性、密钥不进上下文 | 做：`assert_no_secrets` 扩到值模式（D6-10，S6-09） |
| 3.15 / 3.16 | §17.1 CAS 领取、一个 Attempt 一个执行者 | 前序；多池下领取仍单点（S6-08），同 Task 多 Attempt 多工作区（S6-04） |
| 3.17 | §17.3 旧版本 Proposal | 前序（第 5 步 CAS 与重基） |
| 3.18 | §17.4 不重复扣费 | 前序；本步断言 `usage_ref` 重复导入返回 0（S6-07） |
| 3.19 | §17.5 全局 Budget 单写 | 做：Global 账户经 Commit Service（D6-1） |
| 3.20 | §17.6 心跳与 LOST | 前序；多池恢复分区（S6-08） |
| 3.21 | §18.1 预算维度 | 做：tokens / 尝试次数 / 运行时间 / 工具调用 / 费用；登记：GPU 时间、搜索次数、Agent 数量本部署无对应资源 |
| 3.22 | §18.2 分层预算 | 做：Global → Mission → Task → Attempt（D6-1） |
| 3.23 | §18.3 Reserve / Settle | 前序；本步加工具调用维度（D6-8） |
| 3.24 | §18.4 每 Attempt 记录 | 做：模型 / tokens / 工具调用 / 结果状态（trace + costs）；前序：可复用知识与成功路径（第 4 步 lineage）；登记：运行时间由事件时间推出，GPU 无 |
| 3.25 | §18.5 下游慢则上游减速 | 做（S6-02；真实运行 1 两次升起与清除） |
| 3.26 | §18.5 六个降速动作 | 做 4 项：降并发、暂停低优先扩展、禁止继续分裂、缩小预留；不做：提高 Verifier 资源（P2-6 砍掉）、合并重复候选（第 8 步） |
| 3.27 | §18.5 六个上限 | 做：`BackpressureLimits` 单一注册表（D6-2） |
| 3.28 | §18.6 进展信号 | 前序（第 5 步 no_progress）；本步不新增 |
| 3.29 | §19.1 停止条件 | 前序；本步新增 `runtime_unavailable`、`verifier_unavailable` 与各预算维度的停止 |
| 3.30 | §19.2 停滞时换模型 | 做：失败计入升级阶梯（D6-4）；停滞轮数检测为前序 |
| 3.31 / 3.32 / 3.33 | §19.3–19.5 环检测、老化、目标漂移 | 前序；RAISED 期间保留饥饿档；"暂停"是调度层抑制，不改 Task 状态 |
| 3.34 / 3.35 | §20.1–20.2 每 Attempt 工作区、Artifact 字段 | 做：S6-04、`Artifact.workspace`（D6-6） |
| 3.36 | §20.3 合并后再验证 | 部分：候选只有验证通过者成为正式版本（S6-04）；Synthesizer 合并为前序（第 4 步）；候选 Artifact 合并器登记不做 |
| 3.37 | §21.1 网关检查顺序 | 做（D6-7，S6-05 单测断言顺序） |
| 3.38 | §21.2 最小权限 | 做：四方交集；生产部署/财务类权限档归第 7 步 |
| 3.39 | §21.3 外部内容不能改权限 | 做：交集冻结、文本不授权（S6-05）；登记：短期 Capability Token 归第 7 步 |
| 3.40 | §23.1 Trace 字段 | 做：`trace.json`（D6-10，S6-09） |
| 3.41 | §23.2 指标 | 部分：健康、成本、验证通过率、复用、时长、配比；新思路数 / 剪枝率 / 重复率 / 误报率 / 污染率归第 8 步 |
| 3.42 | §23.3 成功路径 | 前序（第 4 步 lineage） |
| 3.43 | §23.4 评测四法 | 不做：第 8 步 |
| 3.44 | §28 第三阶段九项 | 八项本步；Human-in-the-loop 归第 7 步 |
| 3.45 | §29.1 拓扑 | 登记：`verifier_workers` 默认 2；演示与真实运行按承载降为 2 并发、1 个验证并发 |
| 3.46 / 3.47 | §29.2 配比、§29.3 公式 | 配比只观测（D6-11）；公式前序不改 |
| 3.48 / 3.49 | §14.1–14.2 六层验证、领域验证器 | 前序；未部署层提交期拒绝、运行期阻塞（D6-9）；本部署只有格式 / 规则 / Critic / 代码测试四层 |
| 3.50 | §26.3 Attempt Schema | 做：加 `runtime_profile_id`（D6-4） |
| 3.51 / 3.52 | §26.6 事件信封、§26.4 结果信封 | 前序；新事件用同一信封 |
