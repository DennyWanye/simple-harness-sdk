...
def test_a_foreign_tenant_sees_nothing(tmp_path):
    def body(orchestrator, control):
        ...
        stranger = MissionControlV1(orchestrator, tenant_id="other", principal=Principal("other:x", "x"))
        codes = []
        for call in (
    ...
    assert len({message.replace(mission_id, "<id>").replace("mission-does-not-exist", "<id>") for _, message in codes}) == 1
...
def test_snapshot_cursor_and_event_pages_agree(tmp_path):
    async def body(orchestrator, control):
        ...
        with sqlite3.connect(Path(tmp_path) / "evidence" / "orchestrator.db") as db:
            max_seq = db.execute("SELECT MAX(seq) FROM events WHERE mission_id = ?", (mission_id,)).fetchone()[0]
        pages, after = [], 0
        while True:
