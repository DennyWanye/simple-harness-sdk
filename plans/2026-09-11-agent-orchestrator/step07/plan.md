...
| HANDED_OFF | 已交接（outbox 已落盘，带 owner 与租约） | SUCCEEDED、FAILED、UNKNOWN（含租约过期仍无结果，即崩溃） |
| UNKNOWN | 交接后结果不明 | SUCCEEDED（核对 COMPLETED 或人工带证据裁决）、FAILED（人工带证据裁决；或核对 CONFIRMED_NOT_STARTED 但已不能再交接——Mission 已结束、再交接次数用完或再校验不通过，权威核对已证明没有发生）、HANDED_OFF（核对 CONFIRMED_NOT_STARTED，且再交接次数未用完、完整再校验通过；同一幂等键） |
