...
| S3-08 附注（第 4 步 D4-21，2026-09-11） | 修复 Attempt 的预留改为 min(名义份额, 账户剩余 − Critic 份额) | 运行期预留失败只在账户剩余 ≤ Critic 份额时发生；`test_s3_08b` 改为让首个 Attempt 真实耗尽账户（usage 21k > 20k），断言不变 | `test_s3_08b` |
