    ...
    subprocess.run(
        [
            ...
            (
                ...
                "from simple_harness import (ContentBlock,ConversationContinuationInput,"
                "Message,MessageRole,RequestId,canonical_json);"
                ...
                "ContextRouteReceipt,ContextRouteState,DisclosureContext,"
                "EMPTY_WORKSPACE_BINDING_ROOT_SET_DIGEST,LongTermMemoryType,"
                ...
                "RuntimeDecisionSinkPort,TaskExecutionAuthorityPort,TaskExecutionEnvelope,"
                "TaskExecutionEnvelopeRequest,TaskScopeMutationPlan,TaskScopeProposal,"
                "TaskScopeRoute,ToolEffectClass,ToolExecutionPolicy,ToolRouteRequirement,"
                "ToolTaskScopeRequirement);"
                "from simple_harness import workspace_binding_root_set_digest;"
                "assert LongTermMemoryType.SEMANTIC.value=='semantic';"
                "assert TaskScopeRoute.CREATE_NEW.value=='create_new';"
                "assert ContextRouteState.ROUTED_STANDALONE.value=='routed_standalone';"
                "assert ToolEffectClass.PROJECT_EFFECT.value=='project_effect';"
                "assert ToolRouteRequirement.REQUIRED.value=='required';"
                "assert ToolTaskScopeRequirement.REQUIRED.value=='required';"
                "assert workspace_binding_root_set_digest(())"
                "==EMPTY_WORKSPACE_BINDING_ROOT_SET_DIGEST;"
                "legacy=ContextRouteReceipt('legacy','run','raw','effect',"
                "TaskScopeRoute.DIRECT_STANDALONE,None,None,('memory-1',),1);"
                "assert ContextRouteReceipt.from_json(legacy.to_json())==legacy;"
                "public=provider_response_json(ProviderResponse(RequestId('public-1'),"
                "Message(MessageRole.ASSISTANT,(ContentBlock('output_text',"
                "{'text':'visible','provider_private':'WHEEL_PRIVATE_CANARY'}),))));"
                "assert 'WHEEL_PRIVATE_CANARY' not in canonical_json(public);"
