...
def _starve_the_second_ask(loop: Orchestrator) -> None:
    ...
    async def create(mission_id: str, goal_task_id: str, *, ordinal: int, schema_feedback=()):
        if ordinal >= 2:
        ...
        try:
            return await original_create(
                mission_id, goal_task_id, ordinal=ordinal, schema_feedback=schema_feedback
            )
        finally:
...
def test_v2_keeps_its_bytes_stays_registered_and_is_still_pinnable():
    ...
    assert request.to_json()["role_prompt_version"] == "method-synthesizer-v3"
