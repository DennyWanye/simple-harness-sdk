...
async def _invoke(method: str, adapter, transaction, *, changed: bool = False):  # type: ignore[no-untyped-def]
    if method == "mark_running_on_claim":
        return await adapter.mark_running_on_claim(
            transaction,run_id="run",checkpoint_namespace="native",
            lease_epoch=1,claim_epoch=1,now=2.0 if changed else 1.0,
        )
    if method == "consume_decisions":
        return await adapter.consume_decisions(
            transaction,run_id="run",checkpoint_id="checkpoint",
            decision_ids=("decision",),responses={"decision": {"ok": not changed}},
            checkpoint_namespace="native",lease_epoch=1,now=1.0,
        )
    if method == "open_decision":
        return await adapter.open_decision(
            transaction,run_id="run",interrupt_id="interrupt",
            request={"kind":"interrupt","prompt":"changed" if changed else "stable"},
            checkpoint_namespace="native",checkpoint_id="checkpoint",
            lease_epoch=1,now=1.0,
        )
    if method == "materialize_intent":
        return await adapter.materialize_intent(
            transaction,run_id="run",intent_id="intent",
    ...
    if method == "link_effects":
        return await adapter.link_effects(
            transaction,run_id="run",checkpoint_namespace="native",
            checkpoint_id="checkpoint",effect_ids=("effect",),lease_epoch=1,
            now=2.0 if changed else 1.0,
        )
    if method == "finalize_run":
        return await adapter.finalize_run(
            transaction,run_id="run",terminal_checkpoint_id="terminal",
            status="failed" if changed else "completed",
            outcome={"changed":changed},checkpoint_namespace="native",
            lease_epoch=1,now=1.0,
        )
...
METHODS = (
    "mark_running_on_claim", "consume_decisions", "open_decision",
    "materialize_intent", "link_effects", "finalize_run",
)
...
@pytest.mark.parametrize("method", METHODS)
def test_after_commit_response_loss_reopens_receipt_first(
    tmp_path: Path, method: str
) -> None:
...
def test_global_operation_id_rejects_cross_method_second_writer(tmp_path: Path) -> None:
    ...
    try:
        ...
        async def winner(transaction):  # type: ignore[no-untyped-def]
            return await adapter.open_decision(
                transaction,run_id="run",interrupt_id="shared",request={"kind":"interrupt"},
                operation_id=shared_id,checkpoint_namespace="native",
                checkpoint_id="checkpoint",lease_epoch=1,now=1.0,
            )
        asyncio.run(uow.run_atomic(winner,fault_label="winner"))
        async def loser(transaction):  # type: ignore[no-untyped-def]
            return await adapter.materialize_intent(
                transaction,run_id="run",intent_id="shared",
