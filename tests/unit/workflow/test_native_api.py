...
def test_native_constructor_requires_generic_projection_authorities() -> None:
    ...
    assert (
        signature.parameters["terminal_commit_projection_port"].default is inspect.Parameter.empty
    )
...
def test_native_store_projection_prepare_api_snapshot() -> None:
    ...
    assert list(read.parameters) == ["self", "operation_id", "expected_head", "configurable"]
...
def test_projection_context_is_recursively_immutable_and_detached() -> None:
    ...
    context = ProjectionContext("run", "workflow", "1", "checkpoint", source, 10.0, 20.0)
    source["nested"] = {}
